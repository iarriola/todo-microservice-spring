...
public class LoggerUtils {
  public static Function<ServerRequest, ServerRequest> stdoutBefore =
    request -> {
      logger(TaskRouter.class).info("Requesting: {} {}", request.method(), request.path());
      return request;
    };

  public static BiFunction<ServerRequest, ServerResponse, ServerResponse> stdoutAfter = (request, response) -> {
    logger(TaskRouter.class).info("Responding: {} {} {}", request.method(), request.path(), response.statusCode());
    return response;
  };
